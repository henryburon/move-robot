"""
Mover API for moving the franka panda arm to the desired pose.

Raises
------
    RuntimeError: When the tf tree is invalid.

Subscriptions
-------------
    joint_states [sensor_msgs/msg/JointState]: The joint states of the panda arm.
    tf [tf2_msgs/msg/TFMessage]: The transform tree of all frames.
    tf_static [tf2_msgs/msg/TFMessage]: The static transforms.

Clients
-------
    compute_ik [moveit_msgs/srv/GetPositionIK]: Get joint position for desired pose via
                                                inverse kinematics.
    compute_cartesian_path [moveit_msgs/srv/GetCartesianPath]: Get path for moving through
                                                               all requested poses.
    move_group/list_parameters [rcl_interfaces/srv/ListParameters]: Get all parameters for
                                                                    move_group node.
    get_planning_scene [moveit_msgs/srv/GetPlanningScene]: Get the planning scene for motion
                                                           planning.
    apply_planning_scene [moveit_msgs/srv/ApplyPlanningScene]: Apply the new planning scene.

Action Clients
--------------
    move_action [moveit_msgs/action/MoveGroup]: Move the robot to the desired robot state.
    execute_trajectory [moveit_msgs/action/ExecuteTrajectory]: Execute the planned trajectory
                                                               on robot.
    panda_gripper/grasp [franka_msgs/action/Grasp]: Grasp an object on gripper.
    panda_gripper/homing [franka_msgs/action/Homing]: Recalibrate the gripper and move to
                                                      home pose.
...
class State(Enum):
    ...
    ----------
        ...
        CARTPATH: Find cartesian path
        FINDPATH: Finding a path
        EXECUTE: Executing a path
        ...
        HOMING: Performing a homing operation
...
class MoveRobot:
    ...
    -------
        ...
        find_and_execute_cartesian(poses): Finds and execute a cartesian path over all waypoints
        ...
        future_cart_path_callback(future_cart_path): Callback function for asynchronous cartesian
                                                     motion planning
    ...
    def __init__(self, node: Node, move_group_name, fake_mode, ee_frame_id):
        ...
        self.sub_transforms = self.node.create_subscription(
            TFMessage, "tf", self.transform_callback, 10
        )
        ...
        self.sub_tf_static = self.node.create_subscription(
            TFMessage, "tf_static", self.tf_static_callback, qos_profile=qos_tf_static
        )
        while self.node.count_publishers("tf_static") < 1:
            self.node.get_logger().info("waiting for tf_static publisher", once=True)

        self.get_planning_scene = self.node.create_client(
        ...
        while not self.compute_ik.wait_for_service(timeout_sec=2.0):
            self.node.get_logger().info(
                "compute IK Service not available, waiting again..."
            )

        self.computer_cartesian_path = self.node.create_client(
            GetCartesianPath, "compute_cartesian_path", callback_group=self.cb_group
        )
        while not self.computer_cartesian_path.wait_for_service(timeout_sec=2.0):
            self.node.get_logger().info(
                "Get Cartesian Path service not avaiable, waiting again ..."
            )

        self.client_mg_params = self.node.create_client(
            ListParameters, "move_group/list_parameters"
        )
        ...
        while not self.move_action_client.wait_for_server(timeout_sec=2.0):
            self.node.get_logger().info(
                "Move group server not available, waiting again ..."
            )
        ...
        while not self.execute_trajectory_client.wait_for_server(timeout_sec=2.0):
            self.node.get_logger().info(
                "Execute trajectory server not available, waiting again ..."
            )

        if not fake_mode:
            self.gripper_grasp_client = ActionClient(
                self.node, Grasp, "panda_gripper/grasp", callback_group=self.cb_group
            )
            while not self.gripper_grasp_client.wait_for_server(timeout_sec=2.0):
                self.node.get_logger().info(
                    "Grasp server not available, waiting again ..."
                )

            self.gripper_homing_client = ActionClient(
                self.node, Homing, "panda_gripper/homing", callback_group=self.cb_group
            )
            while not self.gripper_homing_client.wait_for_server(timeout_sec=2.0):
                self.node.get_logger().info(
                    "Homing server not available, waiting again ..."
                )

        # Timer
        self.tmr = self.node.create_timer(
    ...
    def timer_callback(self):
        ...
        if self.state == State.STARTUP:
        ...
        elif self.state == State.CARTPATH:
            self.node.get_logger().info("call compute cartesian path", once=True)
            future_cart_path = self.computer_cartesian_path.call_async(
                self.format_cartesian_path_request()
            )
            self.node.get_logger().info("during compute cartesian path", once=True)
            future_cart_path.add_done_callback(self.future_cart_path_callback)
            ...
            future_execute = self.execute_trajectory_client.send_goal_async(
                self.path_msg
            )
            ...
            future_scene = self.get_planning_scene.call_async(
                GetPlanningScene.Request()
            )
            future_scene.add_done_callback(self.future_scene_callback)

        elif self.state == State.BOX:
            future_box = self.apply_planning_scene.call_async(
                ApplyPlanningScene.Request(scene=self.planning_scene)
            )
            ...
            future_homing = self.gripper_homing_client.send_goal_async(self.homing_msg)
    ...
    def homing(self):
        """
        Initiate a homing operation with specified parameters.

        This method sets up homing goal with transitions to the robot's state to HOMING
        """
        self.homing_msg = Homing.Goal()
    ...
    def grasp(self, width, speed, force):
        ...
        Args
        ----
            width (float): The width between two fingers.
            speed (float): The speed which gripper is moving.
            force (float): The force for the gripper to grab the object.
        ...
        self.grasp_msg.width = width
        self.grasp_msg.speed = speed
        self.grasp_msg.force = force
    ...
    def execute_path(self, path):
        ...
        # IGNORE: Find a way to implement the force control
    ...
    def find_and_execute_cartesian(self, poses: list[Pose]):
        """
        Find and execute the cartesian path passing through all requested waypoints.

        This method is used for setup the variable to compute the cartesian path though
        all requested waypoints and excute the computed path.

        Args:
        ----
            poses (list[Pose]): The requestes waypoints list.

        """
        self.waypoints = poses
        self.state = State.CARTPATH
    ...
    def angle_axis_to_quaternion(self, theta, axis):
        ...
        ----
            ...
            axis (tuple[float, float, float]): the rotational axis. This will be normalized

        Returns
        -------
            Quaternion: A Quaternion corresponding to the rotation
        ...
        return Quaternion(
            x=normalized[0] * sinTheta2,
            y=normalized[1] * sinTheta2,
            z=normalized[2] * sinTheta2,
            w=math.cos(theta / 2.0),
        )

    def quaternion_mult(self, q0: Quaternion, q1: Quaternion):
        """
        ...
        Returns
        -------
            Quaternion: The resulting qyaternion

        """
        q = quaternion_multiply((q0.x, q0.y, q0.z, q0.w), (q1.x, q1.y, q1.z, q1.w))
    ...
    def future_cart_path_callback(self, future_cart_path):
        """
        Call back for when the computer_cartesian_path service is done.

        Args:
        ----
            future_cart_path (rclpy.task.Future): The future object for the
            cartesian path service

        """
        self.node.get_logger().info("end compute cartesian path")

        result = future_cart_path.result()

        # if result.fraction < 0.99:
        #     self.node.get_logger().info(f"Fraction: {result.fraction}")
        #     self.try_again = True
        #     self.state = State.CARTPATH
        #     return
        ...
        self.node.get_logger().debug(f"Computed Path: {path}")
    ...
    def future_move_callback(self, future_move):
        ...
        Args:
        ----
            future_move (rclpy.task.Future): The future
    ...
    def future_b_callback(self, future_b):
        ...
        Args:
        ----
            future_b (rclpy.task.Future): The future representing
    ...
    def future_execute_callback(self, future_execute):
        ...
        Args:
        ----
            future_execute (rclpy.task.Future): The future representing
    ...
    def future_c_callback(self, future_c):
        ...
        Args:
        ----
            future_c (rclpy.task.Future): The future representing the
            result of execution.
    ...
    def future_scene_callback(self, future_scene):
        ...
        Args:
        ----
            future_scene (rclpy.task.Future): Future representing result of
    ...
    def future_box_callback(self, future_box):
        ...
        Args:
        ----
            future_box (rclpy.task.Future): Future representing result of
    ...
    def future_grasp_callback(self, future_grasp):
        ...
        Args:
        ----
            future_grasp (rclpy.task.Future): Future representing grasp action request.
    ...
    def future_grasp_d_callback(self, future_e):
        ...
        Args:
        ----
            future_e (rclpy.task.Future): The future representing
    ...
    def future_homing_callback(self, future_homing):
        self.node.get_logger().info("end homing")
        f = future_homing.result()
        future_f = f.get_result_async()
        future_f.add_done_callback(self.future_homing_f_callback)

    def future_homing_f_callback(self, future_f):
        self.node.get_logger().info("end end homing")
        self.state = State.DONE
    ...
    def FormatPositionIKRequest(self):
        ...
        Returns
        -------
            GetPositionIK_Request: computer_ik request object
    ...
    def format_cartesian_path_request(self):
        """
        Format the message that get sent to the compute cartesian path request.
        ...
        --------
            ...
            ----------------------------
            bool: avoid_collisions
            float: max_cartesian_speed
            float: max_velocity_scaling_factor
            float: max_acceleration_scaling_factor
            string: cartesian_speed_limit_link

        Returns
        -------
            GetCartesianPath_Request: compute_cartesian_path request object

        """
        self.node.get_logger().info("Formatting CPath", once=True)
        self.robot_state.joint_state = self.js_current
